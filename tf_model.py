...
class CustomModel:
    ...
    def build_model(self):
        ...
        with tf.variable_scope("encode_x"):
            self.fwd_lstm = core_rnn_cell.BasicLSTMCell(self.h_dim, state_is_tuple=True)
        ...
        # COMPUTE WEIGHTED
        self.r = tf.reshape(tf.matmul(self.att, self.Y, name="r"),shape=[self.batch_size,self.h_dim])  # (nb,1,L) X (nb,L,k) = (nb,1,k)
    ...
    def train(self,\
        ...
        for ITER in range(MAXITER):
            ...
            for i in xrange(0, int((len(xdata)/self.batch_size))*self.batch_size, self.batch_size):
            ...
            total_test_acc=[]
            for i in xrange(0, int((len(xxdata)/self.batch_size))*self.batch_size, self.batch_size):
...
if __name__ == "__main__":
    ...
    print ("num_classes:",options.num_classes)
    Z_train = to_categorical(Z_train, num_classes=options.num_classes)
    Z_dev = to_categorical(Z_dev, num_classes=options.num_classes)
    Z_test = to_categorical(Z_test, num_classes=options.num_classes)
    # print Z_train[0]
